...
#define ADC_UNDER_VOLTAGE_7V 1250U	 // chk 1267 ~ 1271 // 1252~1258
#define ADC_UNDER_VOLTAGE_8_5V 1538U // chk 1576 ~ 1578 // 1551~1554
#define ADC_UNDER_VOLTAGE_9V 1640U	 // chk 1679 ~ 1681 // 1651~1654
#define ADC_OVER_VOLTAGE_16V 3010U	 // chk 3112 ~ 3115 // 3038~3046
#define ADC_OVER_VOLTAGE_16_5V 3105U // chk 3215 ~ 3217 // 3138~3147
#define ADC_OVER_VOLTAGE_18V 3400U	 // chk 3521 ~ 3524 // 3435~3442
...
#define STEP_POSITION_MINIMUM_RANGE 11000U // 12000
// #define STEP_POSITION_MINIMUM_RANGE 130U	//12000
#define STEP_POSITION_MAXIMUM_RANGE 18500U // Close - Open = 11628 30% margin
...
#define POSITION_OPEN_THRESHOLD 60U
...
unsigned int time_1ms_adc_500ms_chk = 0U;
unsigned int time_1ms_adc_500ms_chk_flag = 0U;
...
static unsigned int LIN_ERROR_COUNT = 0U;
...
void main(void)
{
	...
	AAFx_Type = INTERNAL_TYPE; // SX3k AAF1, 2 EXTERNAL_TYPE(UPPER), AAF3 INTERNAL_TYPE(LOWER)

	AAFx_Index = AAF_3; // number 1 aaf
...
static void ERROR_chk(void)
{
	...
	else if ((AAF_ProtectionMode_Rx == OFF) && (stall_test_mode == 0U))
	{
		if (adc_chk_ok_flag == 10U)
		{
			...
			if ((AAFx_Low_Volt == UNDER_VOLTAGE) && (protection_function == ON))
			{
				if (adc_avr >= ADC_UNDER_VOLTAGE_9V)
				{
					...
					time_1ms_adc_500ms_chk = 0;
					time_1ms_adc_500ms_chk_flag = 0;
					Re_Init();
				}
			}
			else if (adc_avr <= ADC_UNDER_VOLTAGE_8_5V)
			{
				if (Under_Voltage_Deceted == 0)
				{
					...
					time_1ms_adc_500ms_chk = 0;
					time_1ms_adc_500ms_chk_flag = 1;
				}
				if ((Under_Voltage_Deceted == 1) && (time_1ms_adc_500ms_chk >= 500))
				{
					AAFx_Low_Volt = UNDER_VOLTAGE;
					protection_function = ON;
					DTC_Status |= 0x20;
					time_1ms_adc_500ms_chk = 500;
					time_1ms_adc_500ms_chk_flag = 0;
				}
			}
			else
			...
			else
			{
				if (adc_avr >= ADC_OVER_VOLTAGE_16_5V)
				{
					...
					if ((Over_Voltage_Deceted == 1) && (time_1ms_adc_500ms_chk >= 500))
					{
						AAFx_Over_Volt = OVER_VOLTAGE;
						protection_function = ON;
						DTC_Status |= 0x40;
		...
		else if (Short_Detected == 1)
		{
			...
			else if ((motor_Short_chk_count >= 10))
			{
				DRV_Off();
				motor_start = OFF;
		...
		else if (Open_Detected == 1)
		{
			...
			else if ((motor_Open_chk_count >= 10))
			{
				DRV_Off();
				motor_start = OFF;
				AAFx_Motor_Fault = 1;
...
void LIN_Short_Chk(void)
{
	Short_chk = PORT.PPR8 & _PORT_Pn1_OUTPUT_HIGH;

	if (Short_chk == 0)
	{
		if (time_1ms_LIN_GndShort_flag == 0)
		{
			time_1ms_LIN_GndShort = 0;
			time_1ms_LIN_GndShort_flag = 1;
		}
	}
	else
	{
		time_1ms_LIN_GndShort_flag = 0;
		time_1ms_LIN_GndShort = 0;
	}

	if (Short_chk == PORT.PPR8 & _PORT_Pn1_OUTPUT_HIGH)
	{
		if (time_1ms_LIN_BatShort_flag == 0)
		{
			time_1ms_LIN_BatShort = 0;
			time_1ms_LIN_BatShort_flag = 1;
		}
	}
	else
	{
		time_1ms_LIN_BatShort_flag = 0;
		time_1ms_LIN_BatShort = 0;
	}

	if ((time_1ms_LIN_GndShort >= 10000) && (time_1ms_LIN_GndShort_flag == 1) || (time_1ms_LIN_BatShort >= 10000) && (time_1ms_LIN_BatShort_flag == 1))
	{
