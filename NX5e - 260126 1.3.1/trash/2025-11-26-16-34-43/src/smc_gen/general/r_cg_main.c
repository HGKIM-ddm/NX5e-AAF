...
static void LIN_GND_Short_Chk(void);
static void OBD_Position_Status(void);
static void OBD_Short_Chk(void);
...
#define ADC_UNDER_VOLTAGE_7V 1243U	// chk 1267 ~ 1271 // SX3 OBD 1239~1243
#define ADC_UNDER_VOLTAGE_8_5V 1538U // chk 1576 ~ 1578 // SX3 OBD 1536~1538
#define ADC_UNDER_VOLTAGE_9V 1635U	// chk 1679 ~ 1681 // 1632 ~ 1635
#define ADC_OVER_VOLTAGE_16V 3011U	// chk 3112 ~ 3115 // 3011~3018
#define ADC_OVER_VOLTAGE_16_5V 3111U // chk 3215 ~ 3217 // 3111 ~ 3114
#define ADC_OVER_VOLTAGE_18V 3404U	// chk 3521 ~ 3524 // 3404 ~ 3410
...
#define ADC_VOLTAGE_13_5V 2581U
#define ADC_VOLTAGE_13_7V 2621U
...
#define ADC_VOLTAGE_14_8V 2841U
#define ADC_VOLTAGE_15V 2881U
...
#define MOTOR_CW_STALL_CHK_VALUE_LOW_VOLTAGE_2ND 100U	   // 135  600 PPS 160
#define MOTOR_CCW_STALL_CHK_VALUE_LOW_VOLTAGE_2ND 100U 		// 145	600 PPS 160

#define MOTOR_CW_STALL_CHK_VALUE_NORMAL_VOLTAGE 100U	   // 135  600 PPS 160
#define MOTOR_CCW_STALL_CHK_VALUE_NORMAL_VOLTAGE 100U 		// 145	600 PPS 160

#define MOTOR_CW_STALL_CHK_VALUE_HIGH_VOLTAGE_1ST 100U	   // 135  600 PPS 160
#define MOTOR_CCW_STALL_CHK_VALUE_HIGH_VOLTAGE_1ST 100U 		// 145	600 PPS 160

#define AAF_ERROR_ANGLE 5U//V
#define AAF_FULL_ANGLE 90U
#define AAF_1ST_OPEN_ANGLE 65U
#define AAF_2ST_OPEN_ANGLE 45U
#define STEP_POSITION_MINIMUM_RANGE 3000U	//12000  
//#define STEP_POSITION_MINIMUM_RANGE 130U	//12000  
#define STEP_POSITION_MAXIMUM_RANGE 16000U
//#define STEP_POSITION_MAXIMUM_RANGE 25500U//15500 //V
...
static uint16_t tx_16bit_spi[11] = {
	...
	0x080F, // [4] (RW)CTRL 2
	0x0A05, // [5] (RW)CTRL 3
	0x0C36, // [6] (RW)CTRL 4	3E (open load on) 0C36
	0x0E10, // [7] (RW)CTRL 5
...
unsigned int time_1ms_EngRunSta_chk=0;
unsigned int time_1ms_EngRunSta_chk_flag=0;
...
static unsigned int wakeup_chk = 1;	// wakeup ?占쏙옙?占쏙옙?占쏙옙
...
static unsigned int automotive_ready = 0;
...
static unsigned int OBD1_tolerance=0U;
static unsigned int OBD2_tolerance=0U;
...
unsigned int time_1ms_LIN_Short_flag = 0U;
unsigned int time_1ms_LIN_Short = 0U;
...
void main(void)
{
	...
	//CFECC_VCI.CFSTCLR_VCI.UINT8[0] = 1;   //
	...
	AAFx_Type = EXTERNAL_TYPE;//SX3k AAF1, 2 EXTERNAL_TYPE(UPPER), AAF3 INTERNAL_TYPE(LOWER)
	...
	while (1)
	{
		...
		if ((step_check_flag == 0) && ((EngRunSta == 0x01) || (HevRdy == 0x01)))//IGN > POSITION CHECK
		{
			...
			if ((time_1ms_step_chk >= 10))
			{
				step_check();
				time_1ms_step_chk_flag = 0;
				time_1ms_step_chk = 10;
				step_check_flag = 1;
			}
...
void RLIN30_receive_complete_interrupt(void)
{
	...
	if ((AAF_LIN_ChkSum_CHK_value == Req_ChkSum_Rx))
	{
...
//-----------------------------------------development START----------------------------------------
/*static void Init_move(void)//Modify
{
	switch (init_move_step)
	{
	case 0:
		//Init_direction(); //development
		...
		wakeup_chk = 0;	// wakeup ?占쏙옙?占쏙옙?占쏙옙
	...
	case 4:
		...
		if (((motor_stall_flag == MOTOR_STALL) || (time_1ms_init_chk >= 4300U))&& AAFx_Type == EXTERNAL_TYPE)
		{
			DRV_Off();
			motor_start = OFF;
		...
		else if (((motor_stall_flag == MOTOR_STALL) || (time_1ms_init_chk >= 4300U)) && AAFx_Type == INTERNAL_TYPE)
		{
			DRV_Off();
			motor_start = OFF;
	...
	case 12:
		...
		open_1st_step_position = (step_position_close - step_position_open) * AAF_1ST_OPEN_ANGLE / AAF_FULL_ANGLE;
		open_2nd_step_position = (step_position_close - step_position_open) * AAF_2ST_OPEN_ANGLE / AAF_FULL_ANGLE;
	...
	case 13:
		if ((step_position <= step_position_open + limit_step_position)&& AAFx_Type == EXTERNAL_TYPE)
		{
			Motor_Close();								 // dir CLOSE
			DRV_On();									 // drv on
			motor_start = ON;							 // step start
		...
		else if ((step_position >= step_position_close - limit_step_position) && AAFx_Type == INTERNAL_TYPE)
		{
			Motor_Open();								 // dir CLOSE
			DRV_On();									 // drv on
			motor_start = ON;							 // step start
	...
	case 14:
		...
		else if ((step_position >= step_position_open + limit_step_position)&& AAFx_Type == EXTERNAL_TYPE)
		{
			DRV_Off();
			motor_start = OFF;
			// step_position_open = step_position;
...
static void Init_move(void)
{
	...
	case 7:
		if ((motor_stall_flag == MOTOR_STALL) || (time_1ms_init_chk >= 4500U))
		{
			...
			OBD1_Close_Check = (OBD1_adc - OBD1_tolerance);
			OBD2_Close_Check = (OBD2_adc - OBD2_tolerance);
	...
	case 10:
		if ((motor_stall_flag == MOTOR_STALL) || (time_1ms_init_chk >= 4500U))
		{
			...
			timer_1ms_init_fail_chk_flag = 0;
			timer_1ms_init_fail_chk = 0;
			OBD1_Open_Check = (OBD1_adc + OBD1_tolerance);
			OBD2_Open_Check = (OBD2_adc + OBD2_tolerance);
...
static void VDC_adc(void)
{
	...
	if (adc_chk_ready == 1U)
	{
		INTC1.ICADCA0I0.BIT.RFADCA0I0 = 0;
		//R_Config_ADCA0_ScanGroup1_GetResult(&bat_adc, 8);
		R_Config_ADCA0_ScanGroup1_GetResult(&scan_results, 3);
		OBD1_adc = scan_results[0];
		OBD2_adc = scan_results[1];
		OBD1_tolerance = (OBD1_adc * OBD_tolerance) /100;
		OBD2_tolerance = (OBD2_adc * OBD_tolerance) /100;
		bat_adc = scan_results[2];
		adc_sum = 0;
...
static void ERROR_chk(void)
{
	...
	else if ((AAF_ProtectionMode_Rx == OFF) && (stall_test_mode == 0U))
	{
		if (adc_chk_ok_flag == 10U)
		{
			if (adc_avr <= ADC_UNDER_VOLTAGE_8_5V)
			{
				AAFx_Low_Volt = UNDER_VOLTAGE;
				protection_function = ON;
				DTC_Status |= 0x20;
				//AAF_ProtectionMode_Tx = ON;	// protection mode Tx NOT ICE
			}
			else if ((AAFx_Low_Volt == UNDER_VOLTAGE) && (protection_function == ON) && (adc_avr >= ADC_UNDER_VOLTAGE_9V))
			{
				AAFx_Low_Volt = NO_ERROR;
				protection_function = OFF;
				//AAF_ProtectionMode_Tx = OFF;	// protection mode Tx NOT ICE
			...
			else if ((AAFx_Over_Volt == OVER_VOLTAGE) && (protection_function == ON) && (adc_avr <= ADC_OVER_VOLTAGE_16V))
			{
				AAFx_Over_Volt = NO_ERROR;
				protection_function = OFF;
				//AAF_ProtectionMode_Tx = OFF;	// protection mode?占쏙옙 vpc ?占쏙옙?占쏙옙
				protection_Mode_step = 0;
		...
		//if (((motor_open_load == MOTOR_FAULT) || (AAFx_InitStatus == ABNORMAL_FINISHED_INITIALIZATION))&&(motor_fault_chk==1))
		...
		if (AAF_OverCurrent == OVER_CURRENT)
		{
			AAFx_Circuit_Short = AAF_CIRCUIT_SHORT;
			protection_function = ON;
			DTC_Status |= 0x04;
			//DTC_memory_write = DTC_memory_write | 0x02;
			//AAF_ProtectionMode_Tx = ON;	// protection mode?占쏙옙 vpc ?占쏙옙?占쏙옙
		}
		else
		{
		...
		LIN_GND_Short_Chk();
		OBD_Short_Chk();
		/*if (motor_open_load == MOTOR_FAULT)
		{
			AAFx_Motor_Fault = 1;
	...
	//
	/*
	if(AAF_HighTemperature == HIGH_TEMPERATURE)
	{
		AAFx_HighTemperature = 1;
		protection_function = ON;
		AAF_ProtectionMode_Tx = ON;
	}
	else
	{
		AAFx_HighTemperature = NO_ERROR;
	}
*/

		if ((AAFx_Over_Volt == NO_ERROR) && (AAFx_Low_Volt == NO_ERROR) && (AAFx_Motor_Fault == NO_ERROR) && (AAF_OverCurrent  == NO_ERROR)&&(AAFx_Circuit_Short == NO_ERROR) && (AAFx_Circuit_Open == NO_ERROR) && (protection_function == ON))
		{
			protection_function = OFF;
	...
	{
		time_1ms_Moving_chk_flag = 1;
		if (time_1ms_Moving_chk >= 10000)
		{
			time_1ms_Moving_chk_flag = 0;
			time_1ms_Moving_chk = 0;
			AAFx_Position_Status = Unknown_Status;
			Re_Init();
		}
	}
...
static void Flash_memory_read(void)
{
	...
	DTC_memory_read = (unsigned int)(r_buff[3] >> 16) & 0xFFFF;

	power_chk_memory_read= (unsigned int)(r_buff[3]) & 0xFFFF;
	First_Powerchk_memory_read = (unsigned int)(r_buff[4]) & 0xFFFF;
	
	OBD1_Close_Check_memory_read = (unsigned int)(r_buff[4]>>16) & 0xFFFF;//4byte 源뚯?;
	OBD1_Open_Check_memory_read = (unsigned int)(r_buff[5]) & 0xFFFF;//4byte 源뚯?;
	OBD2_Close_Check_memory_read = (unsigned int)(r_buff[5]>>16) & 0xFFFF;//4byte 源뚯? ;
	OBD2_Open_Check_memory_read = (unsigned int)(r_buff[6]) & 0xFFFF;//4byte 源뚯? ;
...
static void Operating_mode(void)
{
	...
	case AAF_INITIALIZATION:
		...
		case NORMAL_INITIALIZATION:
			...
			//Flash_memory_write();
			/*if (AAFx_Type == EXTERNAL_TYPE) //Modify
			{
				AAF_Tx_Position = OPEN;
				AAFx_Position_Status = Open_Status;
				AAFx_InitStatus = NORMAL_FINISHED_INITIALIZATION;
			}
			else if (AAFx_Type == INTERNAL_TYPE)
			{
				AAF_Tx_Position = CLOSE;
		...
		if (protection_function == OFF) // 
		{
			...
			if (lin_rx_pass_flag == PASS)
			{
				if (AAFx_InitStatus != DURING_INITIALIZATION)
				{
					...
					/*if ((EngRunSta == 0x00U) && (HevRdy == 0x00U) && (wake_up_motor_range_init_chk == 0U))
					{
						//evrdy_on_flag = OFF;
					}
					else if (((EngRunSta == 0x01U) || (HevRdy == 0x01U)) && (evrdy_on_flag == OFF)&&(AAF_LINOut=0x01U) && (wake_up_motor_range_init_chk == 0U) && (time_1ms_protection_chk == 550U))
						//else if (((EngRunSta == 0x01U) || (HevRdy == 0x01U)) && (evrdy_on_flag == OFF) && (wake_up_motor_range_init_chk == 0))
					{
						// fail_safety_flag = ON;
						DRV_Off();									  // drv of
						motor_start = OFF;							  // step stop
...
default:
	...
	Slave_TxData[2] = (uint8_t)((AAFx_Over_Volt << 6) | (AAFx_Low_Volt << 5) | (AAFx_Motor_Fault << 4) | (AAFx_Circuit_Short << 2) | (AAFx_Circuit_Open << 1));
	Slave_TxData[3] = (uint8_t)((AAFx_SNSR_SCG << 7) | (AAFx_SNSR_SCB << 6) | (AAFx_SNSR_OC << 5) | (AAFx_Position_Status << 2) | AAFx_ErrorStatus);
	Slave_TxData[4] = (uint8_t)(TotalNumOfAAFSensor);
...
static void Lin_sleep(void)
{
	...
	case 4:
		/*if ((motor_stall_flag == MOTOR_STALL) || (timer_1ms_lin_sleep >= 4500U))
		{
			DRV_Off();
			motor_start = OFF;
			stall_chk_cnt = 0;
		...
		//if (((aaf_action == OPEN) && (step_position <= (step_position_open + limit_step_position))) || (motor_stall_flag == MOTOR_STALL))//
		if (((aaf_action == OPEN) && (step_position <= (step_position_open + limit_step_position))))//
		{
			DRV_Off();
			motor_start = OFF;
		...
		else if (motor_stall_flag == MOTOR_STALL)
		{
			...
			AAF_Tx_Position = UNKOWN_POSITION;
			AAFx_Position_Status = Unknown_Status;
			AAFx_InitStatus = INVALID_STATUS_OF_INITIALIZATION;
			motor_stall_flag = MOTOR_NORMAL; // stall reset
			lin_sleep_step = 5;
		}
		else
...
{
	if ((EngRunSta == 0x01) || (HevRdy == 0x01))
	{
		time_1ms_EngRunSta_chk_flag = 1;
	}
	else
	{
		time_1ms_EngRunSta_chk_flag = 0;
		time_1ms_EngRunSta_chk = 0;
	}

	if (time_1ms_EngRunSta_chk >= 500)
	{
		time_1ms_EngRunSta_chk = 500;
	}
	if (time_1ms_EngRunSta_chk == 500)
	{
...
void step_check(void)
{
	...
	//if (((step_position_close - step_position_open) <= STEP_POSITION_MINIMUM_RANGE) || ((step_position_close - step_position_open) > STEP_POSITION_MAXIMUM_RANGE) || (step_position == REFERENCE_POSITION) || (step_position < (step_position_open + limit_step_position)) || (step_position > (step_position_close - limit_step_position)) || (step_position_close == 0) || (step_position_open == 0) || (limit_step_position == 0) || (evrdy_on_flag == OFF) || (step_position > POSITION_MAXIMUM_RANGE) || (step_position_open > POSITION_MAXIMUM_RANGE) || (step_position_close > POSITION_MAXIMUM_RANGE) || (limit_step_position > LIMITSTEP_MAXIMUM_RANGE) || (AAF_Tx_Position == UNKOWN_POSITION) || (AAFx_InitStatus == ABNORMAL_FINISHED_INITIALIZATION) || (AAFx_Position_Status == FlapMoving_Status) || (AAFx_Position_Status == Unknown_Status)||(power_chk== Shutdown_Check))
	{
		...
		Diag_Mode_chk = 0;
		evrdy_on_flag = OFF;
		time_1ms_Error_chk_flag = 0;
		time_1ms_Error_chk = 0;*/
		Re_Init();
	}
	else
	{
		...
		else if (step_position >= (step_position_close - limit_step_position))
				//else if (step_position >= step_position_close)
		{
			aaf_step = AAF_WAITING;
			aaf_init_step = NORMAL_INITIALIZATION;
			AAF_Tx_Position = CLOSE;
...
void OBD_Short_Chk(void)
{

	if ((OBD1_adc > 4000) || (OBD2_adc > 4000))
	{
		time_1ms_OBD_BatShort_flag = 1;
		
		if (time_1ms_OBD_BatShort >= 1000)
		{
			time_1ms_OBD_BatShort = 1000;
			time_1ms_OBD_BatShort_flag = 0;
			AAFx_SNSR_SCB = 1;
			OBD_Short_Bat = 1;
		}
	}
	else if ((OBD1_adc == 0) || (OBD2_adc == 0))
	{
		time_1ms_OBD_GndShort_flag = 1;

		if (time_1ms_OBD_GndShort >= 1000)
		{
			time_1ms_OBD_GndShort = 1000;
			time_1ms_OBD_GndShort_flag = 0;
			AAFx_SNSR_SCG = 1;
			OBD_Short_Gnd = 1;
		}
	}
	else if (((OBD1_adc >0) && (OBD1_adc <= 400)) || ((OBD2_adc >0) && (OBD2_adc <= 400)))
	{
		time_1ms_OBD_Open_Circuit_flag = 1;

		if (time_1ms_OBD_Open_Circuit >= 1000)
		{
			time_1ms_OBD_Open_Circuit = 1000;
	...
	if (((OBD_Short_Gnd == 1) || (OBD_Short_Bat == 1) || (OBD_Open_Circuit == 1)))
	{
		OBD_Error_flag = 1;
		DRV_Off();
		motor_start = OFF;
	...
	else if ((OBD_Error_flag == 1) && ((OBD_Short_Gnd == 0) && (OBD_Short_Bat == 0) && (OBD_Open_Circuit == 0)))
	{
		OBD_Short_Gnd = 0;
		OBD_Short_Bat = 0;
		OBD_Open_Circuit = 0;
...
void LIN_GND_Short_Chk(void)
{
	Short_chk = PORT.PPR8 & _PORT_Pn1_OUTPUT_HIGH;

	if (Short_chk == 0)
	{
		if (time_1ms_LIN_Short_flag == 0)
		{
			time_1ms_LIN_Short = 0;
			time_1ms_LIN_Short_flag = 1;
		}
	}
	else if(Short_chk != 0)
	{
		if (time_1ms_LIN_Short_flag == 1)
		{
			if (time_1ms_LIN_Short < 1000)
			{
				time_1ms_LIN_Short = 0;
				time_1ms_LIN_Short_flag = 0;
			}
		}
	}
	else
...
{
	OBD1_position_percent = ((OBD1_adc - OBD1_Close_Check) * 100) / (OBD1_Open_Check - OBD1_Close_Check);
	OBD2_position_percent = ((OBD2_adc - OBD2_Close_Check) * 100) / (OBD2_Open_Check - OBD2_Close_Check);

	if (((OBD1_Close_Check) > 3600) || ((OBD1_Open_Check) > 3600))
	{
	...
	if (((OBD2_Open_Check) > 3600) || ((OBD2_Close_Check) > 3600))
	{
	...
	else if (AAFx_InitStatus == NORMAL_FINISHED_INITIALIZATION)
	{
		if (step_check_ok == 0)
		{
			if ((SNSR1_Check == USE_SNSR1))
			{
				...
				if ((OBD1_position_percent <= 45))
				{
					AAFx_SNSR1_Position = SNSR1_Close;
				}
				else if (OBD1_position_percent >= 55)
				{
			...
			if ((SNSR2_Check == USE_SNSR2))
			{
				...
				if (OBD2_position_percent <= 45)
				{
					AAFx_SNSR2_Position = SNSR2_Close;
				}
				else if (OBD2_position_percent >= 55)
				{
	...
	if ((AAFx_Position_Status == FlapMoving_Status) && (SNSR_Position_Ok == 0)&&(SNSR1_Check == USE_SNSR1))
	{
		...
		if ((OBD1_position_percent <= 45) && (AAFx_SNSR1_Position != SNSR1_Close))
		{
			AAFx_SNSR1_Position = SNSR1_Close;
			SNSR_Position_Ok = 1;
		}
		else if ((OBD1_position_percent >= 55) && (AAFx_SNSR1_Position != SNSR1_Open))
		{
			AAFx_SNSR1_Position = SNSR1_Open;
			SNSR_Position_Ok = 1;
		}
	...
	if ((AAFx_Position_Status == FlapMoving_Status) && (SNSR2_Position_Ok == 0) && (SNSR1_Check == USE_SNSR2))
	{
		//AAFx_SNSR1_Position = SNSR1_Moving;

		//if ((OBD2_position_percent <= 45) && ((AAF1_TargetPosition_select == OPEN) || (AAF2_TargetPosition_select == OPEN) || (AAF3_TargetPosition_select == OPEN)))
		if ((OBD2_position_percent <= 45) && (AAFx_SNSR2_Position != SNSR2_Close))
		{
			AAFx_SNSR2_Position = SNSR2_Close;
			SNSR2_Position_Ok = 1;
		}
		//else if ((OBD2_position_percent >= 55) && ((AAF1_TargetPosition_select == CLOSE) || (AAF2_TargetPosition_select == CLOSE) || (AAF3_TargetPosition_select == CLOSE)))
		else if ((OBD2_position_percent >= 55) && (AAFx_SNSR2_Position != SNSR2_Open))
		{
			AAFx_SNSR2_Position = SNSR2_Open;
			SNSR2_Position_Ok = 1;
		}
	...
	/*if ((AAFx_Position_Status == FlapMoving_Status) && (SNSR_Position_Ok == 0))//1450 1.2V(1.7V) 3060 3.2V(3.7V) 876-0.5V 4.5V 3798
	{
		if (AAF1_TargetPosition_select == OPEN)
		{
			//if ((OBD1_adc >= 2300) &&(step_position < (step_position_open + middle_step_position)))//OBD1_adc && OBD1_adc
			if ((OBD1_adc >= 2300)&& (OBD1_adc<=2500))//OBD1_adc && OBD1_adc
			{
				AAFx_SNSR1_Position = SNSR1_Open;
				SNSR_Position_Ok = 1;
			}
		...
		else if (AAF1_TargetPosition_select == CLOSE)
		{
			//if ((OBD1_adc <= 2250) &&(step_position >= (step_position_close - middle_step_position)))
			if((OBD1_adc<=2250)&&(OBD1_adc>=2150))
			{
				AAFx_SNSR1_Position = SNSR1_Close;
				SNSR_Position_Ok = 1;
			}
	...
	else if(((AAFx_Position_Status==Open_Status)||(AAFx_Position_Status==Close_Status))&& SNSR_Position_Ok == 1)
	{
		if (AAF_Tx_Position == OPEN)
		{
			AAFx_SNSR1_Position = SNSR1_Open;
